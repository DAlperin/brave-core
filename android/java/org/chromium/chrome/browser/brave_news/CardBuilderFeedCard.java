...
public class CardBuilderFeedCard { // implements ItemClickListener {
    ...
    public CardBuilderFeedCard(
            LinearLayout layout, Activity activity, int position, FeedItemCard newsItem, int type) {
...
*/
    ...
    private void setImage(ImageView imageView, String type) {
        // FeedItemMetadata itemMetaData;
        // switch(mNewsItem.which()){
        //     case FeedItem.Tag.Article:
                
        //         Article article = mNewsItem.getArticle();
        //         FeedItemMetadata articleData = article.data;
        //         itemMetaData = article.data;
        //         // imageView.setText(itemData.title);  
        ...
        //         PromotedArticle promotedArticle = mNewsItem.getPromotedArticle();
        ...
        //         Url paddedImageUrl = itemMetaData.image.getPaddedImageUrl();
        //         if (paddedImageUrl != null){
        //             // Log.d("bn", "createfeed feed item image padded: "+paddedImageUrl.url);
        //             mBraveNewsController.getImageData(paddedImageUrl, imageData -> {
        //                 if (imageData != null){
        ...
        // String imageString = mNewsItem.getImageByte();
        // imageBytes = Base64.decode(imageString, Base64.DEFAULT);
        // Bitmap decodedImage = BitmapFactory.decodeByteArray(imageBytes, 0, imageBytes.length);
        // imageView.setImageBitmap(decodedImage);

        // Log.d("bn", "CardBuilderFeedCard getImageByte:"+mNewsItem);
        // Log.d("bn", "CardBuilderFeedCard getImageByte:"+mNewsItem.getImageByte());
        byte[] imageByte = mNewsItem.getImageByte();
        if (imageByte != null){
    ...
    private void setTextFromFeed(TextView textView, int type) {
    ...
    //             FeedItemMetadata articleData = article.data;
    //             itemData = article.data;
    //             textView.setTextFromFeed(itemData.relativeTimeDescription);  
    //             // braveNewsItems.add(article.data);
    ...
    private void addElementsToSingleLayout(ViewGroup view, int position, int itemType) {
        ...
        switch (itemType) {
            ...
            case THREE_ROWS_PHOTO:
                ...
                setTextFromFeed(source, PUBLISHER);
            ...
            default:
                ...
                setTextFromFeed(title, TITLE);
                setTextFromFeed(desc, DESC);
                layout.addView(image);
                layout.addView(title);
                layout.addView(desc);
                break;
